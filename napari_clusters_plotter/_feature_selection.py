...
class FeatureSelectionWidget(QWidget):
    ...
    def __init__(self, napari_viewer):
        ...
        self.correlation_threshold = create_widget(widget_type="FloatSpinBox",
        ...
        self.analyse_correlation_container.layout().addWidget(self.analyse_correlation_button)
        ...
        def analyse_clicked():
            ...
            if self.correlating_keys != None and len(self.correlating_keys) != 0:
                self.correlation_key_lists = [QListWidget() for correlations in self.correlating_keys]
                for widget,key_list in zip(self.correlation_key_lists,self.correlating_keys):
                ...
                self.correlation_containers = [QWidget() for correlations in self.correlating_keys]
                for i,widget in enumerate(self.correlation_containers):
                    widget.setLayout(QVBoxLayout())
                    widget.layout().addWidget(QLabel("Correlating Group #{}".format(i+1)))
        ...
        self.method_choice_list.currentIndexChanged.connect(self.change_correlation_threshold)
        self.method_choice_list.currentIndexChanged.connect(self.change_correlation_boxes)
    ...
    def change_correlation_threshold(self):
        widgets_inactive(self.correlation_threshold_container, active=self.method_choice_list.currentText() == 'Correlation Filter')
    
    def change_analyse_button(self):
        widgets_inactive(self.analyse_correlation_container, active=self.method_choice_list.currentText() == 'Correlation Filter')

    def change_correlation_boxes(self):
        if self.correlating_keys != None and len(self.correlating_keys) != 0:
            for widget in self.correlation_containers:
                widgets_inactive(widget,active=self.method_choice_list.currentText() == 'Correlation Filter')
