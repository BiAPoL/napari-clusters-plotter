...
class PlotterWidget(QMainWindow):
    def __init__(self, napari_viewer):
        ...
        uic.loadUi(r"C:\Users\savill\OneDrive\Documents\Bia POL stuff\Napari Plugin Development\plotter_inputs.ui", self.control_widget)
        ...
        def plotting_type_changed(): # TODO NEED TO ADD WHICH VARIABLE STORES THE TYPE
            if self.control_widget.plot_type_box.currentText() == PlottingType.HISTOGRAM.name:
                self.control_widget.bins_settings_container.setVisible(True)
                self.control_widget.log_scale_container.setVisible(True)
            elif self.control_widget.plot_type_box.currentText() == PlottingType.SCATTER.name:
        ...
        def bin_auto():
            self.control_widget.manual_bins_container.setVisible(not self.control_widget.auto_bins_checkbox.isChecked())
        ...
        # Adding Connections
        self.control_widget.plot_type_box.currentIndexChanged.connect(plotting_type_changed)
        ...
        self.control_widget.non_selected_checkbutton.stateChanged.connect(checkbox_status_changed)
        self.control_widget.cmap_box.currentIndexChanged.connect(replot)
        self.control_widget.x_axis_box.currentIndexChanged.connect(replot) # TODO Decide if this is a good idea
        self.control_widget.y_axis_box.currentIndexChanged.connect(replot) # TODO Decide if this is a good idea
        ...
        self.hide_non_selected = self.control_widget.non_selected_checkbutton.isChecked()
    ...
    def x_axis(self,column: str):
        self.control_widget.x_axis_box.setCurrentText(column) #TODO insert checks and change values
    @property
    def y_axis(self):
        return self.control_widget.y_axis_box.currentText()
    @y_axis.setter
    def y_axis(self,column: str):
        self.control_widget.y_axis_box.setCurrentText(column) #TODO insert checks and change values
