...
@catch_NaNs
def mean_shift(reg_props: pd.DataFrame, quantile: float = 0.2, n_samples: int = 50) -> (str, np.ndarray):
...
@catch_NaNs
def gaussian_mixture_model(reg_props: pd.DataFrame, cluster_number: int) -> (str, np.ndarray):
...
@catch_NaNs
def kmeans_clustering(reg_props: pd.DataFrame, cluster_number: int, iterations: int) -> (str, np.ndarray):
...
@catch_NaNs
def agglomerative_clustering(reg_props: pd.DataFrame, cluster_number: int, n_neighbors: int) -> (str, np.ndarray):
...
@catch_NaNs
def hdbscan_clustering(reg_props: pd.DataFrame, min_cluster_size: int, min_samples: int) -> (str, np.ndarray):
    import hdbscan
