...
def test_plotter_utilities():
    ...
    spots_clustered = spot_size_clustered(predicts,frame_ids,current_frame,n_datapoints)
    result = [spot_size if pred>=0 else spot_size/2 for pred in predicts]
    result_sc = [result[i] if frame != current_frame else result[i]*frame_spot_f for i,frame in enumerate(frame_ids)]

    assert spots_clustered == result_sc

    spots_unclustered = spot_size_unclustered(frame_ids,current_frame,n_datapoints)
    result_su = [spot_size if frame != current_frame else spot_size*frame_spot_f for frame in frame_ids]
    ...
    result_cc = [result[i] if frame != current_frame else highlight for i,frame in enumerate(frame_ids)]

    assert colors_cl == result_cc

    colors_uc = colors_unclustered(frame_ids,current_frame)
    result_cu = ['#9A9A9A' if frame != current_frame else highlight for i,frame in enumerate(frame_ids)]

    assert colors_uc == result_cu

    cl_plot_params = clustered_plot_parameters(predicts,frame_ids,current_frame,n_datapoints,colors)
