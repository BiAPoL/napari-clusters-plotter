...
MY_WIDGET_NAMES = ["Measure Widget", "Plotter Widget",
...
def test_processing_dask_array(make_napari_viewer):
    ...
    expected_region_props_partial = {'label': np.array([1, 2, 3]),
                                     'area': np.array([20., 20., 12.]),
                                     'mean_intensity':
                                         np.array([44.5, 49.5, 67.5])}
    viewer = make_napari_viewer()
    img_layer = viewer.add_image(dask_image)
    label_layer = viewer.add_labels(dask_label_image)
    region_props = get_regprops_from_regprops_source(img_layer.data,
                                                     label_layer.data,
                                                     region_props_source)
    assert np.all((np.array_equal(expected_region_props_partial['label'],
                                  region_props['label']) and
                   np.array_equal(expected_region_props_partial['area'],
                                  region_props['area']) and
                   np.array_equal(
