...
def generate_cluster_image_from_layer(
    ...
    label_list: ArrayLike
        An array of label IDs in the corresponding order to prediction_list_list
    predictionlist: ArrayLike
    ...
    if isinstance(layer, napari.layers.Labels):
        ...
        output = map_array(
            label_image, np.array(label_list), predictionlist_new
        ).astype("uint64")
        layer_type = "Labels"
...
def dask_cluster_image_timelapse(label_image, label_id_list, prediction_list_list):
    ...
    label_id_list: ArrayLike
        An array of label IDs in the corresponding order to prediction_list_list
    prediction_list_list : ArrayLike
        An array of prediction lists. Each prediction list contains the predicted cluster labels
        for the corresponding frame in the label image.
