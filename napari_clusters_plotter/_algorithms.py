...
def cluster_kmeans(
        data: pd.DataFrame,
        n_clusters: int = 3,
        scale: bool = True) -> FunctionWorker[pd.Series]:
    """
    Cluster the data using KMeans
    """

    @thread_worker(progress=True)
    def _cluster_kmeans(data: pd.DataFrame, n_clusters: int, scale: bool) -> pd.Series:
...
def cluster_hdbscan(
    ...
    @thread_worker(progress=True)
    def _cluster_hdbscan(data: pd.DataFrame, min_cluster_size: int, min_samples: int, scale: bool) -> pd.Series:
...
def  cluster_gaussian_mixture(
    ...
    @thread_worker(progress=True)
    def _cluster_gaussian_mixture(data: pd.DataFrame, n_components: int, scale: bool) -> pd.Series:
...
def cluster_spectral(
    ...
    @thread_worker(progress=True)
    def _cluster_spectral(data: pd.DataFrame, n_clusters: int, scale: bool) -> pd.Series:
